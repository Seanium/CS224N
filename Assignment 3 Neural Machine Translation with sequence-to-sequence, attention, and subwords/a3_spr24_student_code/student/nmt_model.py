...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # 2. 应用 post_embed_cnn
        X_permuted = torch.permute(X, (1, 2, 0))  # (b, e, src_len)
        X_conv = self.post_embed_cnn(X_permuted)
        X = torch.permute(X_conv, (2, 0, 1))  # 恢复原始形状 (src_len, b, e)
        
        # 3. 打包序列并通过编码器
        X_packed = pack_padded_sequence(X, source_lengths)
        enc_hiddens_packed, (last_hidden, last_cell) = self.encoder(X_packed)
        enc_hiddens_padded, _ = pad_packed_sequence(enc_hiddens_packed)
        enc_hiddens = torch.permute(enc_hiddens_padded, (1, 0, 2))  # (b, src_len, h*2)
        
        # 4. 计算解码器初始状态
        # 拼接前向和后向隐藏状态
        last_hidden_concat = torch.cat((last_hidden[0], last_hidden[1]), dim=1)  # (b, 2*h)
        init_decoder_hidden = self.h_projection(last_hidden_concat)  # (b, h)
        
        # 拼接前向和后向单元状态
        last_cell_concat = torch.cat((last_cell[0], last_cell[1]), dim=1)  # (b, 2*h)
        init_decoder_cell = self.c_projection(last_cell_concat)  # (b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_expanded = dec_hidden.unsqueeze(1)  # (b, 1, h)
        e_t = torch.bmm(dec_hidden_expanded, enc_hiddens_proj.transpose(1, 2)).squeeze(1)  # (b, src_len)
        ...
        # 1. 对 e_t 应用 softmax 得到 alpha_t
        alpha_t = F.softmax(e_t, dim=-1)  # (b, src_len)
        
        # 2. 使用 alpha_t 和 enc_hiddens 进行批量矩阵乘法得到注意力输出向量 a_t
        alpha_t_expanded = alpha_t.unsqueeze(1)  # (b, 1, src_len)
        a_t = torch.bmm(alpha_t_expanded, enc_hiddens).squeeze(1)  # (b, 2h)
        
        # 3. 将 dec_hidden 与 a_t 拼接得到 U_t
        U_t = torch.cat([dec_hidden, a_t], dim=1)  # (b, 3h)
        
        # 4. 将组合输出投影层应用于 U_t 得到 V_t
        V_t = self.combined_output_projection(U_t)  # (b, h)
        
        # 5. 应用 tanh 和 dropout 得到 O_t
