...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 3. 将文档分成三部分
        # 随机选择masked_content的长度,平均为truncated长度的1/4
        avg_mask_len = len(truncated) // 4
        mask_len = random.randint(1, avg_mask_len * 2)  # 允许长度在0~2倍平均长度之间变化
        mask_len = min(mask_len, len(truncated) - 2)  # 确保prefix和suffix至少各有1个字符
        
        # 随机选择masked_content的起始位置
        mask_start = random.randint(1, len(truncated) - mask_len - 1)
        
        prefix = truncated[:mask_start]
        masked_content = truncated[mask_start:mask_start + mask_len]
        suffix = truncated[mask_start + mask_len:]
        
        # 4. 重新排列成所需格式
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        # 添加padding使总长度为block_size + 1
