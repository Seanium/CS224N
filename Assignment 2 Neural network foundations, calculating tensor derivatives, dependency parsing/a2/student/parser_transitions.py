...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ["ROOT"]
        self.buffer = sentence[:]  # 创建句子的副本作为buffer
        self.dependencies = []
    ...
    def parse_step(self, transition):
        ...
        match transition:
            ...
            case "LA":  # Left Arc
                if len(self.stack) >= 2:
                    dependent = self.stack[-2]  # 倒数第二个元素
                    head = self.stack[-1]  # 栈顶元素
                    self.dependencies.append((head, dependent))
                    self.stack.pop(-2)  # 移除倒数第二个元素
            case "RA":  # Right Arc
                if len(self.stack) >= 2:
                    head = self.stack[-2]  # 倒数第二个元素
                    dependent = self.stack[-1]  # 栈顶元素
                    self.dependencies.append((head, dependent))
